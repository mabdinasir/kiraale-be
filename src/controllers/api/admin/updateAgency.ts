...
const adminUpdateAgency: RequestHandler = async (request, response) => {
  try {
    ...
    sendSuccessResponse(response, 200, 'Agency updated successfully by admin', {
    ...
    logError(error, 'ADMIN_UPDATE_AGENCY');
    sendErrorResponse(response, 500, `Failed to update agency: ${(error as Error).message}`);
  }
};

export default adminUpdateAgency;
